...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Title */}
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">Overview Dashboard</h1>
      ...
      {isLoadingMetrics ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-white border rounded-xl p-5 sm:p-6 h-28 animate-pulse space-y-3">
              ...
              <Card key={card.title}>
                <CardContent className="p-5 sm:p-6 flex items-center justify-between">
                  <div className="space-y-1.5">
                    <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider">{card.title}</span>
                    <p className="text-2xl sm:text-3xl font-extrabold text-slate-900 leading-none">{card.value}</p>
                  </div>
                  <span className={`p-2.5 sm:p-3.5 rounded-xl ${card.color}`}>
                    <Icon size={20} className="stroke-[2.5] sm:w-[22px] sm:h-[22px]" />
                  </span>
                </CardContent>
              </Card>
            );
